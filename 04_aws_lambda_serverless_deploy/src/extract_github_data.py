...
def parse_args():
    ...
    p.add_argument("--aws-profile", default=os.environ.get("AWS_PROFILE"), help="AWS profile")
...
def lambda_handler(event, context):
    """
    AWS Lambda entry point. Credentials come from the Lambda execution role.

    Environment variables (set on the Lambda function):
        S3_BUCKET    - destination bucket (required)
        AWS_REGION   - region (default: us-east-1)
        TARGET_ROWS  - rows to collect (default: 1000)

    Event dict can override any of the above:
        {"target_rows": 500}
    """
    target_rows = int(event.get("target_rows", os.environ.get("TARGET_ROWS", DEFAULT_TARGET_ROWS)))
    s3_bucket   = event.get("s3_bucket",  os.environ.get("S3_BUCKET",  DEFAULT_BUCKET))
    s3_prefix   = event.get("s3_prefix",  os.environ.get("S3_PREFIX",  DEFAULT_S3_PREFIX))
    aws_region  = event.get("aws_region", os.environ.get("AWS_REGION", DEFAULT_REGION))

    logger.info("Lambda start: target_rows=%s, bucket=%s, region=%s", target_rows, s3_bucket, aws_region)

    token = get_github_token()
    if token:
        logger.info("[bold green]GitHub token found[/] — 5,000 requests/hour", extra={"markup": True})
    else:
        logger.warning("[bold yellow]No token found[/] — 60 requests/hour", extra={"markup": True})

    session = create_session(token)
    rows = collect_repos(session, target_rows)

    if not rows:
        raise RuntimeError("No rows collected")

    upload_to_s3(rows, s3_bucket, s3_prefix, aws_region)

    return {"statusCode": 200, "body": f"Uploaded {len(rows)} rows to s3://{s3_bucket}/{s3_prefix}/"}
